...
(async () => {
    ...
    try {
        ...
        console.warn('⚠️ Google API Token Expired! Email functions will be disabled.');
        console.warn('   To enable email, update refreshToken in config.');
        console.warn('   Bot will continue running without email functionality.');
        emailEnabled = false;
        transporter = null;
    }

    // Create sendMail function with fallback
    async function sendMail({ to, subject, text, html, attachments }) {
        if (!emailEnabled || !transporter) {
            // Log the email that would have been sent
            console.log('📧 [Email Disabled] Would have sent email to:', to);
            console.log('   Subject:', subject);
            console.log('   Text preview:', (text || html || '').substring(0, 100) + '...');
            
            // Also save to a log file for debugging
            const logEntry = {
            ...
            try {
                const logPath = path.join(__dirname, 'email_logs');
                if (!fs.existsSync(logPath)) {
                    fs.mkdirSync(logPath, { recursive: true });
                }
                fs.appendFileSync(
                    path.join(logPath, `email_fallback_${new Date().toISOString().split('T')[0]}.log`),
        ...
        try {
            ...
            try {
                ...
                if (sendError.message && sendError.message.includes('auth')) {
                    console.warn('🔄 Attempting to refresh Google token...');
                    try {
                        const newToken = await OAuth2_client.getAccessToken();
                        accessToken = newToken;
                        
                        // Update transporter with new token
                        transporter = nodemailer.createTransport({
                            host: 'smtp.gmail.com',
                            service: 'Gmail',
                            auth: {
                        ...
                        throw new Error('Email disabled due to token expiration');
                    }
                }
                throw sendError;
            }
        } catch (finalError) {
            console.error('❌ Failed to send email:', finalError.message);
            
            // Final fallback: log the attempt
            console.log('📧 [Email Error Fallback] Email to:', to);
            console.log('   Subject:', subject);
    ...
    global.utils.emailEnabled = emailEnabled;

    // Add a helper function to check email status
    global.utils.isEmailEnabled = function() {
        return emailEnabled;
    };
    ...
    try {
        ...
        console.warn('⚠️ Could not check for new version:', versionError.message);
    }
    ...
    try {
        ...
        console.warn('⚠️ Google Drive setup failed:', driveError.message);
        console.warn('   Bot will continue without Google Drive functionality.');
        // Create a dummy drive object to prevent crashes
        if (!utils.drive.parentID) {
            utils.drive.parentID = null;
        }
    }
    ...
    try {
        ...
        console.error('❌ Failed to start login process:', loginError.message);
        // Wait 5 seconds and retry login
        setTimeout(() => {
            ...
            try {
                ...
                console.error('   Bot will exit. Please check your credentials.');
                process.exit(1);
            }
        }, 5000);
    }
