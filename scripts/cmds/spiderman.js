    ...
    onStart: async function ({ message, event, args }) {
        ...
        if (mention.length === 0) {
            ...
            bal(one, two).then(ptth => {
                message.reply({ body: "it's him🕸️", attachment: fs.createReadStream(ptth) });
            });
        } else {
            const one = mention[1];
            const two = mention[0];
            bal(one, two).then(ptth => {
                message.reply({ body: "he is not me🕸️", attachment: fs.createReadStream(ptth) });
            });
        }
    },
};
...
async function bal(one, two) {
    ...
    const pth = path.join(tempDir, 'spiderman.png'); 
    const img = await jimp.read("https://i.imgur.com/AIizK0f.jpeg");
